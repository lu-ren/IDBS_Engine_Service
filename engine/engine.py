...
class Engine:
    ...
    def retrieve(self, evaluate=True):
        ...
        if self.options['query_type'] == 'binary':
            ...
            if evaluate:
                posSet = numpy.where(self.train_data.gtruth.inmemory == 1)[0]
                trueLabel = numpy.unique (self.train_data.gtruth.inmem_copy [posSet])
                assert (trueLabel.size == 1)
                self.log('Query label is %d'%int(trueLabel))
                self.train_data.gtruth.setClassOfInterest(int(trueLabel))
            ...
            if self.options ['data_type'] == 'InGPU' and self.options.get('test_batchsize') is None:
                ...
                test_result = self.modelWrapper.test_multibatch (
                    ...
                    transductive = True,
                    ranking=True,
                    rank_and_AP=evaluate
                )
                
            if evaluate:
                self.log('performance in inductive set: Average Precision=%f' % test_result ['transductive_AP'])
        else:
            raise NotImplementedError
